...
class Utils_Auth {
    ...
    async favoriteListingGet(userId, res) {
       const user = await User.findById(userId);
       console.log("log inside fav"+ user);
       const favorites = user.favorite.get(user._id);

       console.log("log inside favs"+ favorites);
       return res.status(200).json({
        favorites,
        message: "You are logged in!",
        success: true
    ...
    async favoriteSearch(userId, searchQuery, res) {
        const user = await User.findById(userId);
        user.favoriteSearch.push(searchQuery)
        await user.save();
        if (user !== undefined) {
            return res.status(200).json({
                message: "Search Favorited!",
                status: "success"
            });
        } else {
            return res.status(500).json({
